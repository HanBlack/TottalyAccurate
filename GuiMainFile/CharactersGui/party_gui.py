...
def save_equipped_items():
    ...
    for character in loaded_characters:
        if character.name == selected_character_name:
            character_class = type(character).__name__
            character_file_path = os.path.join(characters_directory_path, f"{character_class}_{character.name}.json")
            character_data = {
...
if loaded_characters:
    ...
    character_combobox.bind("<<ComboboxSelected>>", lambda event: display_equipped_item_stats(character_combobox.get()))
